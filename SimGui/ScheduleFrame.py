...
class ScheduleFrame(Tkinter.Frame):
    ...
    COLOR_SHARED_OK          = "white"
    COLOR_SHARED_ERROR       = "black"
    ...
    def _redrawSchedule(self):
        ...
        frameLength = self.settings.slotframeLength
        if self.settings.switchFrameLengths:
            frameLength = self.settings.currentFrameLength
            # print 'The CURRENT FRAME LENGTH IS %d' % frameLength


        # initialize grid
        if not self.cells:
            for ts in range(frameLength):
                self.cells.append([])
        ...
        for m in self.engine.motes:
            ...
            for (ts,ch,_) in m.getSharedCells():
                color = self.schedule.itemcget(self.cells[ts][ch], "fill")
                if not color:
                    self.schedule.itemconfig(self.cells[ts][ch], fill=self.COLOR_SHARED_OK)
                else:
                    self.schedule.itemconfig(self.cells[ts][ch], fill=self.COLOR_SHARED_ERROR)
    ...
    def _cellCoordinates(self,ts,ch):
        ...
        if self.settings.switchFrameLengths:
            step  = min(
                (self.WIDTH-4)/self.settings.currentFrameLength,
