    ...
    def propagate(self):
        ...
        with self.dataLock:
            ...
            if self.settings.switchFrameLengths:
                # print 'ASN is %d and timeslot is %d' % (self.engine.getAsn(), self.engine.getTimeslot())
                ts = self.engine.getTimeslot()
            else:
                ts  = asn%self.settings.slotframeLength

            arrivalTime = OrderedDict()
                    ...
                    if self.receivers[i]['channel']==transmission['channel']:
                        ...
                        if self.receivers[i]['mote'] in transmission['dmac']:
                            ...
                            if not self.settings.noInterference:
                                ...
                                # gdaneels: if the receiver can interfere with one of the interferers
                                interferenceFlag = 0
                                ...
                                if lockOn == transmission['smac']:
                                    ...
                                    if pdr>=failure:
                                        ...
                                        # print 'propagation error on mote: %d' % self.receivers[i]['mote'].id
                                    ...
                                    # print 'collision error on mote: %d' % self.receivers[i]['mote'].id
